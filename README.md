...
- Symptom: smoke test reports listen is not permitted.
- Cause: restricted runtime environments may block local socket binds.
- Fix: run smoke test on a local machine with normal loopback networking.
...
- Symptom: restore/import fails due unsafe archive path or invalid structure.
- Fix:
  - Use backups generated by Joblio (`npm run backup`).
  - Ensure archive contains the expected top-level data directory.
