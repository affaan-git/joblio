...
function validateTemplateConfig(raw, templateRoot, options = {}) {
  ...
  const maxBytes = Number.isFinite(options.maxBytes) ? Number(options.maxBytes) : DEFAULT_MAX_TEMPLATE_BYTES;
  const rootAbs = path.resolve(templateRoot);
  const rootReal = requireExisting ? fs.realpathSync(rootAbs) : rootAbs;
  ...
  for (const entry of parsed) {
    ...
    if (requireExisting) {
      ...
      let fileReal = '';
      try {
        fileReal = fs.realpathSync(resolved.abs);
      } catch {
        issues.push(`Template file not found: ${resolved.rel}`);
        continue;
      }
      if (!(fileReal === rootReal || fileReal.startsWith(`${rootReal}${path.sep}`))) {
        issues.push(`Template path escapes template root: ${resolved.rel}`);
        continue;
      }
      if (!st.isFile()) {
